...
var queueJSON = []byte(`
{
  "pipeline" : {
    ...
    "queue": {
      "type": "persisted",
      "events_count": 27745,
      "queue_size_in_bytes": 55144264,
      "max_queue_size_in_bytes": 536870912
    },
    "id" : "main"
  }
}
...
func TestPipelineNoQueueStats(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.NotEqual(t, 12, response.Pipeline.Queue.Events)
...
func TestPipelineQueueStats(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, 27745, response.Pipeline.Queue.Events)
}
